...
int main(int argc, char **argv) {
    ...
    for (const auto& f: filesystem::directory_iterator(argv[1])) {
        if (ph_dct_imagehash(f.path().c_str(), tmphash) < 0)
            continue;
        hashlist1.push_back(ph_imagepoint{tmphash, f.path()});
    }
    ...
    for (const auto& f: filesystem::directory_iterator(argv[2])) {
        if (ph_dct_imagehash(f.path().c_str(), tmphash) < 0)
            continue;
        hashlist2.push_back(ph_imagepoint{tmphash, f.path()});
    }

    for (const auto& e1: hashlist1)
        std::cout << e1.id.filename() << ' ' << e1.hash << '\n';
    for (const auto& e1: hashlist2)
        std::cout << e1.id.filename() << ' ' << e1.hash << '\n';
