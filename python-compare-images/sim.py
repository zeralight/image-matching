...
def main():
    ...
    jpeg_filenames = [os.path.join(jpegs_path, f) for f in os.listdir(jpegs_path)]
    tifs_filenames = [os.path.join(tifs_path, f) for f in os.listdir(tifs_path)]
    for jpeg_filename in jpeg_filenames:
        for tif_filename in tifs_filenames:
            logger.debug("%s %s " %(jpeg_filename, tif_filename))
            similarity = image_similarity_bands_via_numpy(jpeg_filename, tif_filename)
            logger.debug("image_similarity_histogram_via_pil => %s "%(similarity))
...
def image_similarity_bands_via_numpy(filepath1, filepath2):
    ...
    for band_index, band in enumerate(image1.getbands()):
        m1 = numpy.array([p[band_index] for p in image1.getdata()]).reshape((1000, 1000))
        m2 = numpy.array([p[band_index] for p in image2.getdata()]).reshape((1000, 1000))
